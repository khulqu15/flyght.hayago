<template>
    <div class="w-full">
        ...
        <div class="mt-4 flex justify-end" v-if="sensorLog.length > 0">
...
export default defineComponent({
    setup() {
        ...
        const exportToExcel = () => {
            const worksheet = XLSX.utils.json_to_sheet(sensorLog.value);
            const workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, worksheet, "Sensor Data");

            XLSX.writeFile(workbook, `SensorData_${new Date().toISOString().replace(/[:.]/g, '-')}.xlsx`);
        };
        ...
        return { canvasContainer, containerThree, orientation, acceleration, gps, exportToExcel, sensorLog }
    }
})
</script>
